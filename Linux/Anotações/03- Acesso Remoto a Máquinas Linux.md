# 🌐 03 — Acesso Remoto a Máquinas Linux

**Autor:** Kleber  
**Curso:** Linux Fundamentals — DIO  
**Tags:** #linux #dio #ssh #aws #ec2 #acesso-remoto

---

Este arquivo consolida os métodos e ferramentas essenciais para conectar, gerenciar e automatizar fluxos de trabalho em instâncias Linux remotas — competência fundamental para a infraestrutura de dados.
...
Para uma conexão bem-sucedida, são necessários três elementos:

| Elemento | Descrição |
|---|---|
| **O Host** | Endereço IP Público ou DNS da instância |
| **O Usuário** | Definido pela imagem do SO (`ubuntu` para Ubuntu, `ec2-user` para Amazon Linux) |
| **A Identidade** | Arquivo de chave privada que autentica o acesso |

> ⚠️ **Segurança:** A conexão só ocorre se o Security Group permitir o tráfego do seu IP atual na porta 22. No Linux/WSL, a chave `.pem` deve ter permissão restrita:
> ```bash
> chmod 400 chave.pem
> ```
